...
# An elevator is a mechanical device that moves people or goods between different levels in a building. Elevators provide convenient and efficient vertical transportation, enhancing accessibility, saving time, and maximizing space in tall buildings. They come equipped with safety features to ensure passenger well-being.
...
# Requirements are typically categorized into functional and non-functional. However, before categorizing, it's essential to gather these requirements. Here are some pivotal questions to present to the interviewer:

# How many elevators are in the system?
# How do the elevators process requests? Are requests accepted both internally and externally?
# How are the requests prioritized? Is it directional or based on proximity?
# How are intermediate requests addressed?
# Regarding state management, how does the elevator determine its direction and current activity?
# Should the elevator initiate in an IDLE state?
# How is safety ensured within the elevator?
# How does the elevator maintain efficiency?
# From a user experience perspective, what feedback does the elevator provide?
# What measures are in place to emulate the elevator's real-world behaviors?
...
class Request:
    def __init__(self, origin, origin_floor, destination_floor=None):
        self.origin = origin
        self.direction = State.IDLE.value[0]
        self.origin_floor = origin_floor
        self.destination_floor = destination_floor
        self.elevator_tpe = ElevatorType.PASSENGER.value[0]

        if destination_floor is not None:
            if origin_floor > destination_floor:
                self.direction = State.DOWN.value[0]
            elif origin_floor < destination_floor:
                self.direction = State.UP.value[0]
    ...
    def get_elevator_type(self):
        return self.elevator_tpe
    
    def _lt_(self, other):
        return self.destination_floor < other.destination_floor
    
class ServiceRequest(Request):
    def __init__(self, origin, origin_floor=None, destination_floor=None):
        super().__init__(origin, origin_floor, destination_floor)
        self.elevator_tpe = ElevatorType.SERVICE.value[0]
